          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center max-w-4xl mx-auto">
            <div className="inline-block mb-6">
              <div className="w-24 h-24 bg-white/20 backdrop-blur-sm rounded-2xl flex items-center justify-center mx-auto border-2 border-white/30">
                <svg className="w-12 h-12 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
              </div>
            </div>
            <h1 className="text-5xl md:text-6xl font-bold mb-6">{t('contactPage.title')}</h1>
            <p className="text-xl md:text-2xl text-purple-100 leading-relaxed">{t('contactPage.subtitle')}</p>
          </motion.div>
        </Container>
      </section>
        ...
        <Container>
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="text-center mb-12">
            <div className="inline-block mb-4">
              <span className="text-purple-600 font-bold text-sm uppercase tracking-widest bg-purple-50 px-6 py-2 rounded-full border-2 border-purple-200">{t('contactPage.channelsTag')}</span>
            </div>
            <h2 className="text-4xl md:text-5xl font-bold mb-6 bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">{t('contactPage.reachUsTitle')}</h2>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto leading-relaxed">{t('contactPage.channelsDescription')}</p>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {contactMethods.map((method, index) => (
              <motion.a key={index} href={method.href} target={method.href.startsWith('http') ? '_blank' : undefined} rel={method.href.startsWith('http') ? 'noopener noreferrer' : undefined} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }} viewport={{ once: true }} className="group">
                <Card className="p-8 text-center h-full relative overflow-hidden border-2 border-transparent hover:border-gray-200 transition-all hover:shadow-2xl bg-white">
                  ...
                  <div className="relative inline-flex mb-6">
                    ...
                    <div className={`relative ${method.bgColor} text-white p-5 rounded-2xl group-hover:scale-110 transition-transform duration-300`}>{method.icon}</div>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }}>
              <div className="mb-8">
                ...
                <p className="text-lg text-gray-600">{t('contactPage.formDescription')}</p>
              </div>
              <Card className="p-8 md:p-10 border-2 border-gray-100 hover:border-purple-200 transition-all hover:shadow-2xl">
                <ContactForm onSubmit={handleSubmit} labels={{ name: t('contactForm.name'), email: t('contactForm.email'), phone: t('contactForm.phone'), subject: t('contactForm.subject'), message: t('contactForm.message'), submit: t('contactForm.submit'), submitting: t('contactForm.submitting'), success: t('contactForm.success'), error: t('contactForm.error') }} />
              </Card>
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="space-y-6">
              <Card className="h-96 overflow-hidden border-2 border-gray-100 hover:border-purple-200 transition-all">
                <div className="w-full h-full bg-gradient-to-br from-purple-50 via-purple-100 to-pink-100 flex items-center justify-center relative group">
                  <div className="absolute inset-0 opacity-10">
                    <div className="absolute inset-0" style={{ backgroundImage: 'radial-gradient(circle at 20px 20px, rgba(139, 92, 246, 0.5) 2px, transparent 0)', backgroundSize: '50px 50px' }} />
                  </div>
              ...
              <Card className="p-8 border-2 border-transparent hover:border-purple-200 transition-all hover:shadow-2xl relative overflow-hidden group">
                ...
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-2xl font-bold text-gray-900">{t('contactPage.workingHoursTitle')}</h3>
                </div>
                <div className="space-y-4">
                  <div className="flex justify-between items-center p-3 bg-purple-50 rounded-lg">
                    <span className="font-bold text-gray-900">{t('contactPage.weekdays')}:</span>
                    <span className="text-purple-600 font-semibold">{t('contactPage.weekdaysHours')}</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-pink-50 rounded-lg">
                    <span className="font-bold text-gray-900">{t('contactPage.saturday')}:</span>
                    <span className="text-pink-600 font-semibold">{t('contactPage.saturdayHours')}</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-red-50 rounded-lg">
                    <span className="font-bold text-gray-900">{t('contactPage.sunday')}:</span>
                    <span className="text-red-600 font-semibold">{t('contactPage.closed')}</span>
                  </div>
                </div>
              </Card>
            </motion.div>
